...
SELECT COURSENAME FROM COURSE WHERE DEPARTMENTID=(SELECT DEPARTMENTID FROM DEPARTMENT WHERE DEPARTMENTNAME='COMPUTER SCIENCE');

--Display all students who belong to the "Electronics" department.

SELECT NAME FROM STUDENTS WHERE DEPARTMENTID=(SELECT DEPARTMENTID FROM DEPARTMENT WHERE DEPARTMENTNAME='ELECTRONICS');
...
SELECT S.NAME,D.DEPARTMENTNAME FROM STUDENTS AS S INNER JOIN DEPARTMENT AS D ON S.DEPARTMENTID=D.DEPARTMENTID;

--LEFT JOIN Courses with Departments (to show all courses, even if some department does not exist).

SELECT C.COURSENAME,D.DEPARTMENTNAME FROM COURSE AS C LEFT JOIN DEPARTMENT AS D ON C.DEPARTMENTID=D.DEPARTMENTID;

--RIGHT JOIN Students with Courses (to show all courses, even if no student has enrolled in them)

SELECT S.NAME,S.STUDENTID,C.COURSENAME,C.COURSEID FROM STUDENTS AS S  RIGHT JOIN COURSE AS C ON S.DEPARTMENTID=C.DEPARTMENTID;
